# Hi, I'm Hayat 👋

Software Engineering student interested in frontend and backend development, with experience building responsive web applications. I enjoy solving problems, learning new technologies, and improving my development skills through real-world projects.

## 🚀 Tech Stack

### Frontend
- HTML
- CSS
- JavaScript
- TypeScript
- React
- Tailwind CSS

### Backend
- Node.js
- Express.js

### Database
- MongoDB

### Tools
- Git
- GitHub
- VS Code
- Postman

## 📌 Featured Projects

### 🛒 LuxeCart E-commerce Platform
Built a full-stack e-commerce web application using React, TypeScript, Node.js, Express, and MongoDB during the CSEC-ASTU Bootcamp, featuring authentication, responsive UI, shopping cart functionality, AI assistance, and an admin dashboard.

### 🗳️ Blockchain-Based Voting System
Collaborated in a team project to develop a secure and transparent blockchain-based voting application using blockchain technology for vote verification and integrity.


## 🌱 Currently Learning

- Advanced React
- TypeScript
- Backend Development with Node.js
- Software Architecture

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=HayatAbdulfetah&show_icons=true&theme=radical&locale=en" alt="GitHub Stats" />
</p>  

#🔥 Contribution Streak  

<p align="center">
  <img src="https://streak-stats.vercel.app?user=HayatAbdulfetah&theme=radical&border_radius=10&date_format=M%20j%5B,%20Y%5D" alt="GitHub Streak" /></p>


## 🤝 Connect With Me

- GitHub: https://github.com/HayatAbdulfetah
- LinkedIn: https://www.linkedin.com/in/hayat-abdulfetah-598b9a348/

